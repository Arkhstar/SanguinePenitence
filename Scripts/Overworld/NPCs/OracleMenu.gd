...
func init() -> void:
	var display_name : String = "JEROD, THE ORACLE" if SaveData.townsfolk & 1 else "THE ORACLE"
	if await dialogue.speak(display_name, "HARKEN UNTO ME, MY CHILD: EVENTIDE HATH COME.", ["TALK", "EXIT"]) == 0:
		...
		var option : int = await dialogue.speak(display_name, "AND I WILL GRANT YOU AUDIENCE. YOU, NOBLE HUNTER, MAY KNOW ALL THAT YOU REQUIRE.", dialogue_options)
		while option != dialogue_options.size() - 1:
			if option == 0: # THE REALM
				if SaveData.highest_altar_level >= 2500:
					...
					await dialogue.speak(display_name, "AFTER ALL, THERE ARE NONE BUT YOU WHO'VE KEPT THE LIGHT OF PROVIDENCE STATIONED ABOVE OUR HUMBLE VILLAGE.")
				elif SaveData.highest_altar_level >= 500:
					await dialogue.speak(display_name, "FEW FARE WELL THESE TIMES, AS MOST FARE NOT AT ALL.")
					await dialogue.speak(display_name, "YOU CHAMPION THE CAUSE OF FREEDOM, AND WE STAND TALL BEHIND YOU")
				else:
					await dialogue.speak(display_name, "THE TRUMPET HATH SOUNDED, AND THE WATERS RUN BLACK WITH BLOOD. WE STAND ATOP THE PRECIPICE OF LIFE AND SQUARE AGAINST THE EMBRACE OF DEATH.")
					await dialogue.speak(display_name, "STALWART WE LIVE, AGAINST THE WAVE OF NIGHT. AND YOU, %s, BRAVE THE LASHING WAVES MORE THAN MOST." % SaveData.hunter_name)
					await dialogue.speak(display_name, "THROUGH BLOOD AND COIN WILL SALVATION FIND YOU. OF THIS I SWEAR.")
			elif option == 1: # THE TOWN
				if SaveData.highest_altar_level >= 1200:
					await dialogue.speak(display_name, "THE ALCHEMIST HAS SEEN HIMSELF FIT TO GRACE THE SKY WITH HIS PRESENCE ONCE MORE.")
					await dialogue.speak(display_name, "I KNOW NOT WHAT PURPOSE HIS ELIXIRS MAY SERVE, BUT IF YOU SEE THEM FIT FOR USE I WILL NOT OPPOSE YOU.")
				elif SaveData.highest_altar_level >= 450:
					await dialogue.speak(display_name, "A VAGRANT SORCERER SOUGHT SANCTUM FROM THE EVENTIDE. IN EXCHANGE, SHE OFFERED INSIGHTS INTO HER MAGICKS - A BARGAIN NONE COULD REFUSE.")
					await dialogue.speak(display_name, "SPELLCRAFT IS A MIGHTY AND DANGEROUS THING. THAT SHE WOULD OFFER IT SO FREELY BESPEAKS HER DIRE STRAITS.")
				elif SaveData.highest_altar_level >= 50:
					await dialogue.speak(display_name, "I HAVE HAD WORD WITH THE SMITH, AND JOINED HER TO OUR CAUSE.")
					await dialogue.speak(display_name, "THE SANDS OF TIME WILL DULL YOUR BLADE. I IMPLORE YOU TO ACQUAINT YOURSELVES - HER SERVICES WILL BE OF UPMOST USE TO YOU.")
				else:
					await dialogue.speak(display_name, "FEW ARE FIT TO FLEE THEIR WALLS IN SUCH TUMULTUOUS TIMES. THEY HIDE AWAY IN FEAR OF THE DARKNESS LOOMING BEYOND THEIR DOORS.")
				...
				if SaveData.altar_level >= 1000 and not (SaveData.townsfolk & 1):
					...
					await dialogue.speak(display_name, "YOU ARE THE BLADE, AS I AM THE MOUTH.")
				else:
